...
- A horizontal idea tree built from `docs/demo_data/sanitized_tree.json`
...
- A journey timeline plus an interaction timeline derived from sanitized logs
  in `docs/demo_data/fpilot_interactions.json`, illustrating how the
...
3. Write updated JSON / JS bundles under `docs/demo_data/` in the same schema
   as the existing `sanitized_*.json` files.

For public distribution, only the already-sanitized artifacts in
`docs/demo_data/` are needed.

## Preview locally

- Open `docs/index.html` directly in a browser (all data is loaded from
  `demo_data/sanitized_data.js`).
- Or run a local server: `cd docs && python -m http.server 8000` and visit
  http://localhost:8000.
